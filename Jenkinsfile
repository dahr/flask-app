  ...
  "DOCKER_CONTENT_TRUST_REPOSITORY_PASSPHRASE=${ContentTrustRepoPassphrase}"]){
    ...
    withEnv(['DOCKER_CONTENT_TRUST=1']){
      ...
      stage('Set k8s image') {
        ...
        sh "kubectl set image deployment voting-app voting-app=dell-harbor.dell.ecore.af.smil.mil/sdtf/vote_app:latest -n default --kubeconfig=/home/smbl/.kube/config --record=true"
        }
      }
    }
  }
